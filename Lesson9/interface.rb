...
class Interface
  ...
  def add_station
    ...
    begin
      ...
      puts "There were #{attempt} attempts to create a station."
    end
  ...
  def find_car(train, car_number)
    ...
    puts selected_car ? 'Car is found' : 'Car is not found'
    selected_car
  end
  ...
  def modify_route
    if Route.all.empty?
      ...
      if route_existance(route_number).nil?
        ...
        puts 'Select the number of your action: '
        action_number = gets.chomp
        route_action(action_number, selected_route)
      end
    end
  end

  def route_action(action_number, selected_route)
    if action_number == '1'
      print_stations_list
    ...
    if Train.all.empty?
      ...
      if selected_train.nil?
        ...
        if selected_train.tooken_route.nil?
          ...
          puts 'Select the number of your action: '
          action_number = gets.chomp
          move_train_action(action_number, selected_train)
        end
      end
    end
  end

  def move_train_action(action_number, selected_train)
    if action_number == '1'
  ...
  def station_existance(search_station_name)
    ...
    Station.all.each do |station|
      new_station = station if station.station_name == search_station_name
    end
    new_station
  end

  def route_existance(route_number)
    @hash_routes[route_number] if @hash_routes.key?(route_number)
  end
  ...
  def print_cars(train)
    ...
    if train.type == 'cargo'
      train.each_train { |car, index| puts "car number: #{index + 1} - type: #{car.type} - volume: #{car.volume} - occupied volume: #{car.occupied_volume} - free volume: #{car.free_volume}" }
    else
  ...
  def routes_list
    ...
    Route.all.each_with_index do |route, index|
      @route = route
      @hash_routes[(index + 1).to_s] = route
